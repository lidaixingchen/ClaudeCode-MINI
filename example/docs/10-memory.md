...
3. **为什么记忆召回使用 `sideQuery`（旁路查询）让另一个 LLM 调用筛选，而不是直接把所有记忆文件加载进上下文？**
   *(提示：如果用户积累了 100 条记忆，全部加载会占用大量 Token，而且大部分与当前任务无关。sideQuery 用一次轻量级 API 调用（256 tokens）筛选出最相关的几条记忆，实现了“精准召回”而非“暴力加载”，既节省 Token 又提高相关性。)*
