...
> **💡 为什么本课只实现 Anthropic 后端？**
>
> Anthropic 的流式 API 提供了明确的 `content_block_stop` 事件，当工具参数块生成完毕时会立即触发回调，我们可以**实时知道**工具参数已经完整，从而在模型还在输出后续文本时就开始执行工具（抢跑）。
>
> 而 OpenAI 的流式 API 处理方式不同——工具调用参数是通过多个 `delta.tool_calls` 片段逐步累加的，没有明确的”块结束”信号，需要在流结束后才能组装完整的工具调用。因此 OpenAI 的流式输出将在下一课（第 07 课）中实现，届时会使用 `asyncio.gather` 进行并行执行，而非本课的实时抢跑机制。
...
然后在 `agent.py` 中补充导入。除了新增的 `CONCURRENCY_SAFE_TOOLS`，还需要导入后续步骤会用到的 UI 渲染函数：
...
首先在 `agent.py` 顶部定义辅助函数 `_get_max_output_tokens`，它根据模型名称动态返回最大输出 token 数，避免硬编码 4096。不同模型的输出能力差异很大，opus-4-6 可达 64000，而较小的模型则为 16384：

```python
# agent.py 顶部的辅助函数（在 load_dotenv() 之后）
...
接下来需要在 `AgentState` 中添加 `thinking_mode` 属性，因为 `_call_anthropic_stream` 会根据它决定 `max_tokens`：
...
同时在 `Agent.__init__` 中添加 `_system_prompt` 快捷引用：

```python
self._system_prompt = self.history.system_prompt  # 系统提示词的快捷引用
```

#### 为什么需要快捷引用？

**`self._system_prompt`** 是内存中的可变副本，用于快速访问当前系统提示词。这样在 API 调用时可以直接使用 `self._system_prompt`，而不需要每次都从 `self.history` 获取。

**使用方式**：
```python
# 读取时：直接使用快捷引用
response = await self._client.messages.create(
    system=self._system_prompt,  # 快速访问
    ...
)
```

> **💡 设计模式说明**：这是一种"内存副本 + 按需同步"的设计模式。当需要修改系统提示词时，先修改 `self._system_prompt`，然后调用 `self.history.update_system_prompt(self._system_prompt)` 同步到底层存储。第 12 课引入 Plan Mode 时会展示更复杂的使用场景。
...
首先在 `Agent` 类中添加 `_execute_tool_call` 封装方法：
...
接下来修改 `_chat_anthropic` 循环，注册 `_on_tool_block_complete` 回调逻辑：
...
- **权限检查将在第 8 课添加**：目前只读工具直接允许执行，第 8 课会引入 `check_permission` 函数进行权限校验。
...
3. **为什么抢跑机制只允许 `CONCURRENCY_SAFE_TOOLS`（只读工具）参与，而写入类工具（如 `write_file`、`run_shell`）不能抢跑？**
   *(提示：抢跑意味着工具可能在 LLM 还没输出完就开始执行了。如果 LLM 后续修改了意图或参数，已经执行的写入操作无法撤销——比如先写了一个文件，然后 LLM 决定改写另一个文件。只读操作没有副作用，即使抢跑了也不会造成不可逆的影响。)*
