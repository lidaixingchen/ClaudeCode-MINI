...
随后，在 `main()` 函数中实例化 `Agent` 的位置，将预算参数传递给 `Agent`：
...
首先更新 `Agent.__init__` 以接受预算参数：

```python
# agent.py -> Agent.__init__

class Agent:
    def __init__(self, *, backend: BackendConfig, permission_mode="default",
        thinking=False, max_cost_usd=None, max_turns=None, confirm_fn=None,
        custom_system_prompt=None, custom_tools=None, is_sub_agent=False):
...
class AgentState:
    ...
    current_turns: int = 0  # 当前会话已执行的轮次
```
...
#### 注意什么
`clear_history()` 会重置 `total_input_tokens` 和 `total_output_tokens` 为 0，同时清空消息历史。这与 Plan 审批流程中的 `history.clear()` 不同——后者仅清空消息但不清零 Token 计数，因此 Plan 审批后的 Token 仍然持续累加。
...
3. **为什么预算检查（`_check_budget`）要在工具执行之前，而不是在每轮循环结束后？**
   *(提示：如果在循环结束后检查，当前轮次的工具已经执行完毕、API 已经调用，Token 已经消耗。提前检查可以在“即将超限”时立即刹车，避免多花一轮的 API 费用。这是“先检查再执行”的防御性编程思想——就像信用卡额度检查是在消费前，而不是消费后。)*
