...
def _edit_file(inp: dict) -> str:
    ...
    try:
        ...
        return f"Successfully edited {inp['path']}{note}\n\n{diff}"
    except Exception as e:
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "timeout": {"type": "number", "description": "Optional timeout in seconds for the command execution."},
            },
            "required": ["command"],
        },
    },
...
- **不好的做法**：
  ...
  def _write_file(inp: dict) -> str:
      ...
      _auto_update_memory_index(inp["path"])
  ```
- **推荐的做法**：
  ```python
  def _write_file(inp: dict) -> str:
...
3. **为什么 `_truncate_result` 采用"头尾保留、中间截断"的策略，而不是只保留开头或只保留结尾？**
   *(提示：文件开头通常是 import 语句和类定义，结尾通常是报错信息或测试结果——这两部分对 LLM 理解上下文最关键。中间的样板代码往往是冗余的。如果只保留开头，LLM 看不到错误；如果只保留结尾，LLM 缺少上下文。)*
