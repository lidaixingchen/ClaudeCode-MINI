...
3. **为什么系统提示词要在每次 LLM 调用前”动态编译”，而不是在 Agent 初始化时一次性生成？**
   *(提示：用户可能在对话过程中修改 `CLAUDE.md` 文件、切换 Git 分支、或在 `.claude/rules/` 目录添加新规则。动态编译确保 Agent 始终使用最新的配置，无需重启程序。这也是”热更新”思想在 Prompt 工程中的体现。)*
