...
3. **为什么技能要有 `context: fork` 模式？fork 模式和普通模式有什么本质区别？**
   *(提示：普通模式下，技能的 Prompt 直接注入当前对话上下文，模型在同一个会话中执行。fork 模式会创建一个独立的子 Agent，拥有隔离的上下文和工具集——适合执行“原子任务”（如代码审查、提交），避免污染主对话的历史消息。fork 模式的核心优势是“上下文隔离”。)*
